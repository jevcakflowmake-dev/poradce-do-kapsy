...
/**
 * Finanční plán, jak ho vidí klient. Poradce stejnou komponentu otevírá jako
 * náhled (`nahled`, s `klientId`): data klienta, ale bez výběru variant,
 * reakcí a dotazů – ty patří klientovi – a i dřív, než plán zveřejní.
 */
export default function FinancniPlan({ klientId, nahled = false }: { klientId?: string; nahled?: boolean }) {
  ...
  // Klient plán vidí až po zveřejnění (migrace 017); do té doby „Plán se připravuje“.
  const [zverejneno, setZverejneno] = useState(false)
  const [analyzaHotova, setAnalyzaHotova] = useState(false)
  ...
  const loadData = useCallback(async () => {
    ...
    const [variantsRes, recsRes, interestRes, selectionRes, financialsRes, analyzaRes, profilRes] = await Promise.all([
      ...
      supabase.from('client_financials').select('monthly_income_net, age, retirement_age, expected_state_pension, has_mortgage, mortgage_remaining_amount').eq('client_id', id).maybeSingle(),
      supabase.from('analysis_responses').select('section, question_id, value').eq('client_id', id).in('section', ['retirement', 'personal', 'income', 'housing']),
      supabase.from('profiles').select('full_name, plan_zverejnen_at, onboarding_completed').eq('id', id).maybeSingle(),
    ])

    const profil = profilRes.data as { full_name: string | null; plan_zverejnen_at: string | null; onboarding_completed: boolean } | null
    setJmenoKlienta(profil?.full_name ?? null)
    setZverejneno(Boolean(profil?.plan_zverejnen_at))
    setAnalyzaHotova(Boolean(profil?.onboarding_completed))
  ...
  // Nezveřejněný plán klient nevidí; poradce v náhledu ano.
  const hasPlan = planSections.length > 0 && (zverejneno || nahled)
  const allInterested =
    ...
    <div>
      ...
      <header className="bez-tisku mb-10">
        ...
        <p className="mt-3 text-base text-slate">
          ...
          {/* Datum jen u plánu, který klient vidí – u rozdělaného by prozradilo, že se na něm pracuje od kdy. */}
          {hasPlan && planDatum ? ` · ${formatDate(planDatum)}` : ''}
        </p>
      ...
      {loading ? (
        ...
        <div className="bg-surface rounded-card border border-line p-12 md:p-16 text-center">
          ...
          {analyzaHotova ? (
            <p className="text-base text-slate max-w-md mx-auto text-pretty">
              Analýzu mám a na plánu pracuju. Jakmile bude hotový, pošlu vám e-mail a najdete ho tady.
            </p>
          ) : (
            <>
          ...
          <div className="rounded-card bg-navy text-cream p-6 md:p-8 mb-10">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5">
              ...
              <div className="bez-tisku flex flex-col sm:flex-row gap-2.5 shrink-0">
                <Button type="button" onClick={handleBulkInterest} disabled={nahled || bulkLoading || allInterested}>
                  {bulkLoading ? (
                  ...
                  {section.id === 'income' ? (
                    ...
                    <IncomeLifeChart
                      ...
                      onSelect={async (variantId) => {
                        if (nahled) return showToast('V náhledu se nevybírá – variantu vybírá klient.')
                        if (!clientId) return
                      ...
                      return (
                        <div className="space-y-5">
                          ...
                          <VyberVarianty
                            ...
                            selectedId={vybrana}
                            onSelect={(id) =>
                              nahled ? showToast('V náhledu se nevybírá – nabídku vybírá klient.') : vyberJedinou(section, id)
                            }
