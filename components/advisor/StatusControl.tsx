...
export default function StatusControl({ clientId, initial }: Props) {
  ...
  return (
    <div className="relative inline-block">
      ...
      {open && (
        <div
          className="absolute z-20 mt-2 min-w-[220px] right-0 bg-surface border border-line rounded-card shadow-xl p-1.5"
          role="listbox"
        >
          {CLIENT_STATUS_VALUES.map((s) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => change(s)}
                className="w-full flex items-center justify-between gap-3 px-3 py-2 rounded-card hover:bg-cream text-left"
              >
      ...
      {error && (
        <div className="absolute left-0 mt-2 text-xs text-danger bg-[rgba(194,65,12,0.08)] border border-[rgba(194,65,12,0.3)] rounded-card px-2.5 py-1.5 whitespace-nowrap">
          {error}
        </div>
      )}
    </div>
