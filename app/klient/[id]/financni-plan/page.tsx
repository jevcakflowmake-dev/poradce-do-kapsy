...
const planSections = [
  { title: 'Zajištění příjmů', icon: Shield, status: 'recommendation' as const, gradient: 'from-blue-600 to-indigo-700', items: ['Navýšení životního pojištění na 3 mil. Kč', 'Přidání pojištění pracovní neschopnosti od 15. dne', 'Přidání pojištění invalidity 3. stupně'] },
  { title: 'Bydlení', icon: HomeIcon, status: 'ok' as const, gradient: 'from-emerald-600 to-teal-700', items: ['Pojištění nemovitosti a domácnosti je v pořádku', 'Zvážit refinancování hypotéky — úrok 4,2 % je nad trhem'] },
  { title: 'Příprava na důchod', icon: Clock, status: 'action' as const, gradient: 'from-amber-500 to-orange-600', items: ['Zvýšit příspěvek na DPS z 500 na 1 700 Kč', 'Vyjednat příspěvek zaměstnavatele', 'Zahájit pravidelnou investici do ETF fondu'] },
  { title: 'Děti', icon: Baby, status: 'recommendation' as const, gradient: 'from-pink-500 to-rose-600', items: ['Založit stavební spoření pro každé dítě', 'Sjednat úrazové pojištění dětí'] },
  { title: 'Investice', icon: TrendingUp, status: 'action' as const, gradient: 'from-violet-600 to-purple-700', items: ['Spořicí účet: nechat 3 měsíce výdajů jako rezervu', 'Přebytek přesunout do vyváženého portfolia', 'Investiční horizont 10+ let: doporučen dynamický profil'] },
  { title: 'Pojištění majetku', icon: Building2, status: 'ok' as const, gradient: 'from-cyan-600 to-sky-700', items: ['Povinné ručení a havarijní pojištění v pořádku', 'Zvážit pojištění odpovědnosti z běžného života'] },
]
  ...
  return (
    <div className="w-full container px-4 mx-auto max-w-4xl py-8">
      ...
      {!hasPlan ? (
        ...
        <>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-2xl p-6 md:p-8 mb-8 relative overflow-hidden">
          ...
          <div className="space-y-4">
            {planSections.map((section, idx) => {
              ...
              return (
                <motion.div key={section.title} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + idx * 0.08 }} className="bg-white rounded-2xl border border-slate-200 p-5 hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-3 mb-4">
                    <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${section.gradient} flex items-center justify-center shadow-sm`}><section.icon className="w-5 h-5 text-white" /></div>
