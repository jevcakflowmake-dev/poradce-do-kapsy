...
const sections = [
  {
    id: 'income', title: 'Zajištění příjmů', icon: Shield, color: 'from-[#162459] to-[#243471]',
    questions: [
  ...
  {
    id: 'housing', title: 'Bydlení', icon: Home, color: 'from-[#009EE2] to-[#0079AD]',
    questions: [
  ...
  {
    id: 'retirement', title: 'Příprava na důchod', icon: Clock, color: 'from-[#162459] to-[#243471]',
    questions: [
  ...
  {
    id: 'children', title: 'Děti', icon: Baby, color: 'from-[#009EE2] to-[#0079AD]',
    questions: [
  ...
  {
    id: 'investing', title: 'Investice', icon: TrendingUp, color: 'from-[#162459] to-[#243471]',
    questions: [
  ...
  {
    id: 'property', title: 'Pojištění majetku', icon: Building2, color: 'from-[#009EE2] to-[#0079AD]',
    questions: [
  ...
  {
    id: 'personal', title: 'Osobní údaje', icon: UserCircle, color: 'from-[#162459] to-[#0e1a3d]',
    questions: [
...
export default function AnalyzaPage() {
  ...
  return (
    // Pruh nad sazbou, ne fotka vedle ní: sloupec je `max-w-shell` (obsah 1 600 px),
    // takže na běžných obrazovkách vyplní celou šířku – stejně jako u klientské verze.
    <div className="relative max-w-shell mx-auto px-6 md:px-10 lg:px-16 xl:px-20 pt-[9.5rem] md:pt-[15rem] pb-10 md:pb-14">
      ...
      <div className="space-y-4">
        {sections.map((section, sIdx) => {
          ...
          return (
            ...
            >
              <button onClick={() => toggleSection(section.id)} className="w-full flex items-center gap-4 p-5 md:p-6 text-left">
                <div className={`w-11 h-11 rounded-none bg-gradient-to-br ${section.color} flex items-center justify-center flex-shrink-0 shadow-sm`}>
                  <section.icon className="w-5 h-5 text-white" strokeWidth={1.8} />
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="font-display text-[#162459]" style={{ fontSize: '1.1rem', letterSpacing: '-0.01em' }}>
                    {section.title}
                  </h3>
                ...
                {isExpanded ? <ChevronUp className="w-5 h-5 text-[#66708C] flex-shrink-0" /> : <ChevronDown className="w-5 h-5 text-[#66708C] flex-shrink-0" />}
              </button>
              <AnimatePresence>
                {isExpanded && (
                    ...
                    <div className="p-5 md:p-7 space-y-5">
                      ...
                      <div>
                        ...
                        >
                          ...
                          <span className="text-sm text-[#66708C] group-hover:text-[#162459]">Klikněte pro nahrání PDF nebo fotky</span>
                        </button>
                        {sectionFiles.length > 0 && (
                          <div className="mt-2.5 space-y-1.5">
                            {sectionFiles.map(f => (
                              <div key={f.name} className="flex items-center gap-2 bg-[#F6F4EE] rounded-none px-3 py-2.5 text-sm border border-[#E4DFD2]">
                                ...
                                <button onClick={() => removeFile(f.name)} className="text-[#66708C] hover:text-[#c2410c]"><X className="w-4 h-4" /></button>
                              </div>
                            ))}
                          </div>
                        )}
      ...
      <motion.div className="mt-10 flex justify-end" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}>
        <button
          ...
          style={{ background: '#162459' }}
        >
          {loading ? 'Odesílám…' : 'Odeslat analýzu'} <ArrowRight className="w-4 h-4" />
        </button>
      </motion.div>
    </div>
  )
